...
public class ProductRepository {
	...
	//(싱글턴 패턴)
	// 클래스의 멤버 변수로 자기 자신을 갖고있음.
	private static ProductRepository productRepository;
	static {
		productRepository = new ProductRepository();
	}
	//(싱글턴 패턴)
	// 클래스 멤버 변수로 선언되어있는 자기자신(이 클래스의 인스턴스)을
	// 반환하는 메서드
	public static ProductRepository getInstance() {
			return productRepository;
	}
	
	// 접근제어자가 private 인 생성자 의미?
	// 클래스의 외부에서 인스턴스를 만들 수 없음 
	private ProductRepository() {
		Product phone = new Product("P1234", "iPhone 6s", 800000);
		phone.setDescrption("4.7-inch, 1334X750 Retina HD display, 8-megapixel iSight Camera");
		...
		notebook.setDescrption("13.3-inch, IPS LED display, 5rd Generation Intel Core processor");
		...
		tablet.setDescrption("212.8*125.6.6.6mm, Super AMOLED display, Octa-Core processor");
